...
# ── Firewall: open serve port temporarily if nftables/iptables active ────────
if command -v nft &>/dev/null 2>&1; then
    info "Opening port $SERVE_PORT in nftables (input chain)..."
    sudo nft add rule inet filter input tcp dport $SERVE_PORT accept 2>/dev/null \
        && info "Port opened in nftables" \
        || warn "nft rule add failed — may already be open or no 'filter input' chain"
fi
